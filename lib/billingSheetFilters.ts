...
/** Bộ lọc tab trên bảng thu — `pending` = chưa chốt CSM (không chỉ PENDING). */
export type BillingSheetStatusFilter = "all" | "pending" | "confirmed";

const VALID: BillingSheetStatusFilter[] = ["all", "pending", "confirmed"];
...
export function matchesBillingSheetStatusFilter(
  row: Pick<BillingSheetRow, "status">,
...
export function countBillingSheetStatusFilter(
  rows: Pick<BillingSheetRow, "status">[],
  filter: BillingSheetStatusFilter
): number {
